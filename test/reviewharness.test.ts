...
function fakeHarness(over: Partial<{ log: string; ok: boolean; onRun: (dir: string) => void }> = {}): Harness {
  return {
    id: 'fake',
...
test('a log with no findings object yields none', () => {
  assert.deepEqual(parseReviewFindings('the model said nothing useful'), []);
});
