...
test('a type parameter added with a default is a feature, not a break', () => {
  // Measured on activepieces, and the reason this matters more than a Tier 3
  // bug: this is the default path, no flags. axios 1.18 -> 1.19 added `P = any`
  // to `AxiosRequestConfig`, `AxiosResponse` and `isAxiosError`, and all three
  // were reported breaking. Every existing use still compiles — `Config` and
  // `Config<Foo>` both bind, because the new parameter has a default. axios is
  // in nearly every TypeScript repository, so this fires almost everywhere.
  const changes = diffSurfaces(
    surface('1.0.0', [sym('Config', 'Config<D = any>')]),
    surface('2.0.0', [sym('Config', 'Config<D = any, P = any>')]),
  ).changes;
...
test('a type parameter added without a default still breaks', () => {
  // `Config<Foo>` no longer binds: the second parameter has nothing to fall
  // back to. This is the case the rule must not swallow.
  const changes = diffSurfaces(
    surface('1.0.0', [sym('Config', 'Config<D = any>')]),
    surface('2.0.0', [sym('Config', 'Config<D = any, P>')]),
  ).changes;
  assert.equal(changes[0]?.severity, 'breaking');
});

test('a defaulted type parameter does not excuse the rest of the signature', () => {
  // The limit. Adding a defaulted parameter is provably compatible on its own;
  // it says nothing about a value parameter that changed alongside it, and
  // claiming otherwise would trade this false positive for a false negative.
  const changes = diffSurfaces(
    surface('1.0.0', [sym('read', '<T = any>(path: string) => T')]),
    surface('2.0.0', [sym('read', '<T = any, P = any>(path: number) => T')]),
  ).changes;
  assert.equal(changes[0]?.severity, 'breaking');
});
